...
namespace GymProject.App_Code
{
    public class OrderClassesLogic
    {
        ...
        {
               string sql = string.Format("SELECT OrderClasses.CodeDayTime, OrderClasses.chargeId, DayTime.code, Charge.chargeId , Charge.UserId , Subscribers.ID AS ID, Subscribers.[First name] AS firstName, Subscribers.[Last name] AS lastName FROM (((OrderClasses INNER JOIN Charge ON  OrderClasses.chargeId =Charge.chargeId) INNER JOIN Subscribers ON Subscribers.ID=Charge.UserId)INNER JOIN DayTime ON DayTime.code=OrderClasses.CodeDayTime) WHERE OrderClasses.CodeDayTime={0} ", codeDayTime);
               return dal.excuteQuery(sql);
        }
        /*
